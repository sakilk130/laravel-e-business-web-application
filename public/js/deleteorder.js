...
$(document).on("click", ".delete", function () {
    ...
    let modal = $("#deletedelivered");

    // Delete Confirmation
    $('#deletedelivered button[type="submit"]').click(
        {
            id: task,
        },
        call_ajax
    ...
    function call_ajax(event) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/delete_delivered_orders/" + id,
            success: function (data) {
                // reqest message clear
                $(msg).html("");

                $("#deletedelivered").find("h4").remove();
                $("#deletedelivered").find('button[type="submit"]').remove();
